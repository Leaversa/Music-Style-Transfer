...
class MusicVAE(nn.Module):
    ...
    def encode(self, x):
        ...
        h_last = h[
            -1
        ]  # Use the last hidden state from the final LSTM layer (batch, hidden_size)
...
def generate_music_with_temperature(
    model,
    token_to_idx,
    idx_to_token,
    latent_size=64,
...
if __name__ == "__main__":
    ...
    vocab_size = len(
        token_to_idx
    )  # token_to_idx should be available from your training process
    embed_size = 128
    hidden_size = 256
    latent_size = 64
    ...
    loaded_model.load_state_dict(
        torch.load("music_vae.pth", map_location=torch.device("cpu"))
    )
    ...
    generated_tokens_with_temperature = generate_music_with_temperature(
        loaded_model,
        token_to_idx,
        idx_to_token,
        latent_size=64,
    ...
    tokens_to_midi(
        generated_tokens_with_temperature,
        time_resolution=0.05,
        default_duration=1.0,
        output_path="generated_output_with_temperature.mid",
    )
